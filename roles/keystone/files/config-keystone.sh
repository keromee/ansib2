...
if  [ -f /etc/yum.repos.d/*OpenStack*.repo ]
then
...
mysql -uroot -p${ALL_PASSWORD} -e "CREATE DATABASE keystone;" &&  mysql -uroot -p${ALL_PASSWORD} -e "GRANT ALL PRIVILEGES ON keystone.* TO 'keystone'@'localhost' IDENTIFIED BY '${ALL_PASSWORD}';" && mysql -uroot -p${ALL_PASSWORD} -e "GRANT ALL PRIVILEGES ON keystone.* TO 'keystone'@'%' IDENTIFIED BY '${ALL_PASSWORD}';"   && mysql -uroot -p${ALL_PASSWORD} -e "flush privileges;"   
